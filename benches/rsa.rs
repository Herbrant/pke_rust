...
fn rsa_encrypt(bencher: Bencher, m: u64) {
    let mut rng = RandState::new();
    let (_, pk) = RSA::keygen(black_box(SECURY_LEVEL), black_box(&mut rng)).unwrap();

    let m = m.to_le_bytes();

    bencher.bench_local(|| {
        let _ = RSA::encrypt(&pk, &m, &mut rng).unwrap();
    });
}
...
fn rsa_decrypt(bencher: Bencher, m: u64) {
    let mut rng = RandState::new();
    let (sk, pk) = RSA::keygen(black_box(SECURY_LEVEL), black_box(&mut rng)).unwrap();

    let m = m.to_le_bytes();
    let c = RSA::encrypt(&pk, &m, &mut rng).unwrap();

    bencher.bench_local(|| {
        let _ = RSA::decrypt(&pk, &sk, &c).unwrap();
    });
}
